...
class StateManager:
    ...
    def __init__(self, kernel_name: Optional[str] = None):
        self.kernel_name = kernel_name  # 커널 이름 (예: "sub", "add")
    ...
    def set_kernel_name(self, name: str):
        ...
        # 커널 이름이 바뀌면 새 세션 시작 (상태를 START로 변경)
        if self.kernel_name != normalized_name:
            self.kernel_name = normalized_name
            self.reset()  # reset()이 상태를 START로 변경
        # 커널 이름이 같아도 상태가 END이면 새 세션 시작
...
"""
    ...
    def reset(self):
        ...
        # kernel_name은 유지 (reset 후에도 같은 커널이므로)
        current_kernel_name = self.kernel_name

        self.session_id = str(uuid.uuid4())[:8]
        self.status = Status.START
        self.kernel_versions = []
        self.write_count = 0
        self.kernel_name = current_kernel_name  # kernel_name 유지
        self.first_write_reached = False
        self.test_code_written = False
        self.info_collected = {
            "get_overview": False,
            "get_torch_op_info": False,
            "get_triton_syntax": False,
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.json_log_file = self.log_dir / f"{self.session_id}_{timestamp}.jsonl"
        # md_log_file은 set_kernel_name에서 다시 설정되므로 None으로 설정
    ...
    def transition_to(self, new_status: Status, reason: str = "") -> bool:
        ...
        # Track first write state reach
        if new_status == Status.WRITE and not self.first_write_reached:
            self.first_write_reached = True
    ...
    def add_kernel_version(
        self, code: str, kernel_file: str, test_file: Optional[str] = None
    ) -> int:
        """
        ...
        # 마크다운 로그에 기록
        self._append_md_log(
            f"""### [{datetime.now().strftime('%H:%M:%S')}] 커널 v{version} 작성
...
"""
    ...
    def update_kernel_validation(self, version: int, passed: bool, details: str = ""):
        ...
        self._append_md_log(
            f"""### [{datetime.now().strftime('%H:%M:%S')}] 커널 v{version} 검증: {status}
...
"""
    ...
    def update_kernel_timing(self, version: int, mean_ms: float, min_ms: float, max_ms: float):
        ...
        self._append_md_log(
            f"""### [{datetime.now().strftime('%H:%M:%S')}] 커널 v{version} 성능 측정
...
# Ops별 StateManager 저장
_state_managers: Dict[str, StateManager] = {}
_current_kernel_name: Optional[str] = None


def get_state_manager(kernel_name: Optional[str] = None) -> StateManager:
    """
    Get StateManager for a specific kernel.

    If kernel_name is None, uses the current kernel name.
    If no StateManager exists for the kernel, creates a new one.
    If the StateManager's kernel_name differs from the requested name, resets to START.

    Args:
        kernel_name: 커널 이름 (예: "add", "mul", "div")

    Returns:
        StateManager instance for the kernel
    """
    global _state_managers, _current_kernel_name

    # kernel_name이 None이면 현재 커널 이름 사용
    if kernel_name is None:
        kernel_name = _current_kernel_name

    # kernel_name이 여전히 None이면 기본값 사용
    if kernel_name is None:
        kernel_name = "default"

    # 정규화된 이름 사용
    normalized_name = kernel_name.lower().replace(" ", "_")

    # StateManager가 없으면 생성
    if normalized_name not in _state_managers:
        _state_managers[normalized_name] = StateManager(normalized_name)

    state_manager = _state_managers[normalized_name]

    # StateManager의 kernel_name이 요청한 이름과 다르면 리셋
    if state_manager.kernel_name != normalized_name:
        state_manager.kernel_name = normalized_name
        state_manager.reset()  # 상태를 START로 변경

    return state_manager


def set_current_kernel_name(kernel_name: str) -> None:
    """현재 작업 중인 커널 이름을 설정합니다."""
    global _current_kernel_name
    normalized_name = kernel_name.lower().replace(" ", "_")
    _current_kernel_name = normalized_name
...
def get_state_manager_for_op(op_name: str) -> Optional[StateManager]:
    """특정 op의 StateManager를 반환합니다."""
    normalized_name = op_name.lower().replace(" ", "_")
    return _state_managers.get(normalized_name)
...
def reset_all_states() -> int:
    ...
    count = len(_state_managers)

    # 모든 StateManager 리셋
    for state_manager in _state_managers.values():
        state_manager.reset()

    # 현재 커널 이름 초기화
    _current_kernel_name = None
...
def log_tool_call(allowed_statuses: Optional[List[Status]] = None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Also log to markdown log
            state._append_md_log(
                f"""### [{datetime.now().strftime('%H:%M:%S')}] 도구 호출: {func.__name__}

- **상태**: {status_before} → {status_after}
- **성공**: {'✅' if success else '❌'}
{f"- **에러**: {error_msg}" if error_msg else ""}
